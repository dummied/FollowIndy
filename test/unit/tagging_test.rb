...
class TaggingTest < ActiveSupport::TestCase
  fixtures :tags, :taggings, :things
  def setup
    @objs = Thing.find(:all, :limit => 2)
  ...
  def test_find_tagged_with
    ...
    result1 = [@obj1]
    assert_equal Thing.tagged_with("ipa"), result1
    assert_equal Thing.tagged_with("ipa lager"), result1
    assert_equal Thing.tagged_with("ipa", "lager"), result1
    
    result2 = [@obj1.id, @obj2.id].sort
    assert_equal Thing.tagged_with("seasonal").map(&:id).sort, result2
    assert_equal Thing.tagged_with("seasonal lager").map(&:id).sort, result2
    assert_equal Thing.tagged_with("seasonal", "lager").map(&:id).sort, result2
  end
